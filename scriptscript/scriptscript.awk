...
function out_handlers(    str, i, end) {
    out_open_tag(TAG_USER_EVENTS)
    end = in_get_num_rules()
    for (i = 1; i <= end; ++i) {
        str = in_get_rule_name(i)
        out_open_function(out_get_handler_name(str))
        out_line(sprintf("%s()", DATA_OR_ERR_FNAME), 1)
    ...
    out_open_function(AWK_END)
        out_line(sprintf("if (%s() != %s)",
            GET_LAST_RULE_FNAME,
            out_get_rule_name(in_get_rule_name(in_get_num_rules()))), 1)
            out_line(sprintf("%s(\"last read rule not last declared\")",
                IN_ERROR_FNAME), 2)
    out_close_block()
    out_line()
    
    out_open_function(IN_ERROR_FNAME, "error_msg")
...
function out_utils_check_data() {
    out_open_function(DATA_OR_ERR_FNAME)
    out_line("if (NF < 2)", 1)
...
function out_utils(    i, end, arr_input, tmp, tmp_arr, tmp_num) {
    ...
    out_utils_check_data()
    out_line()
        
    end = in_get_num_rules()
    out_open_function("reset_all")
    for (i = 1; i <= end; ++i)
        out_line(sprintf("reset_%s()", in_get_rule_name(i)), 1)
    out_close_block()
    out_line()
    
    out_line(sprintf("function %s() {return %s}",
        GET_LAST_RULE_FNAME, CURRENT_STATE_VAR_NAME))
    
    for (i = 1; i <= end; ++i) {
        tmp = in_get_rule_name(i)
        
        tmp_arr = sprintf("__%s_arr__", tmp)
        tmp_num = sprintf("__%s_num__", tmp)
        
        out_line()
        out_string(sprintf("function save_%s(%s) {", tmp, tmp))        
        out_line(sprintf("%s[++%s] = %s}", tmp_arr, tmp_num, tmp))
        
        out_string(sprintf("function get_%s_count() {", tmp))
        out_line(sprintf("return %s}", tmp_num))
        
        out_string(sprintf("function get_%s(num) {", tmp))
        out_line(sprintf("return %s[num]}", tmp_arr))
        
        out_string(sprintf("function reset_%s() {", tmp))
        out_line(sprintf("delete %s; %s = 0}", tmp_arr, tmp_num))
    }
...
function out_state_change(tabs) {
    out_line(sprintf("%s = %s", CURRENT_STATE_VAR_NAME, NEXT_STATE), tabs)
}
...
function out_end() {
    ...
    out_line("END {")
        out_line(sprintf("if (!%s)", GLOBAL_ERR_FLAG), 1)
            out_line( AWK_END "()", 2)
    out_close_block()
...
BEGIN {    
    ...
    DATA_OR_ERR_FNAME = "data_or_err"
    GET_LAST_RULE_FNAME = "get_last_rule"
